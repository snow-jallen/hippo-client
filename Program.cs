...
namespace HippoClient
{
            ...
            while(true)
            {
                ...
                switch(k.Key)
                {
                    case ConsoleKey.LeftArrow:
                        await gameClient.MoveLeft();
                        break;
                    case ConsoleKey.RightArrow:
                        await gameClient.MoveRight();
                        break;
                    case ConsoleKey.UpArrow:
                        await gameClient.MoveUp();
                        break;
                    case ConsoleKey.DownArrow:
                        await gameClient.MoveDown();
                        break;
                }
            }
        }
    ...
    public class GameClient
    {
        ...
        public async Task MoveLeft() => await httpClient.GetAsync($"{serverAddress}/move/left?token={Token}");
        public async Task MoveRight() => await httpClient.GetAsync($"{serverAddress}/move/right?token={Token}");
        public async Task MoveUp() => await httpClient.GetAsync($"{serverAddress}/move/up?token={Token}");
        public async Task MoveDown() => await httpClient.GetAsync($"{serverAddress}/move/down?token={Token}");
        ...
        public static async Task<GameClient> CreateClientAsync(string serverAddress, string playerName)
        {
            ...
            {
                throw new UnableToConnectToServerException("Unable to connect to server.", ex);
            }
